...
static long is_exec_adbd(struct pt_regs *regs)
{
	...
	if (ret < 0) {
		pr_debug(
			"Access filename when adb_root_handle_execve failed: %ld\n",
			ret);
		return ret;
	}
...
static long setup_ld_preload(struct pt_regs *regs)
{
	...
	if (ret != 0) {
		pr_debug(
			"write ld_preload when adb_root_handle_execve failed: %ld\n",
			ret);
		return -EFAULT;
	}
	...
	ret = copy_to_user(ld_library_path_p, kLdLibraryPath,
			   sizeof(kLdLibraryPath));
	if (ret != 0) {
		pr_debug(
			"write ld_library_path when adb_root_handle_execve failed: %ld\n",
			ret);
		return -EFAULT;
	}
	...
	for (;;) {
		tmp_env_p2 =
			krealloc(tmp_env_p,
				 (env_count + kReadEnvBatch + 2) * kPtrSize,
				 GFP_KERNEL);
		if (tmp_env_p2 == NULL) {
		...
		ret = copy_from_user(&tmp_env_p[env_count],
				     envp + env_count * kPtrSize,
				     kReadEnvBatch * kPtrSize);
		if (ret < 0) {
			pr_debug(
				"Access envp when adb_root_handle_execve failed: %ld\n",
				ret);
			ret = -EFAULT;
			goto out_release_env_p;
		}
		size_t read_count = kReadEnvBatch * kPtrSize - ret;
		size_t max_new_env_count = read_count / kPtrSize,
		       new_env_count = 0;
