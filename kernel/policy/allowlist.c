...
static bool profile_valid(struct app_profile *profile)
{
	...
	if (profile->allow_su) {
		...
		size_t len = strnlen(profile->rp_config.profile.selinux_domain,
				     domain_len);
...
int ksu_set_app_profile(struct app_profile *profile)
{
	...
	if (profile->allow_su) {
		pr_debug(
			"set root profile, key: %s, uid: %d, gid: %d, context: %s\n",
...
void ksu_put_root_profile(struct root_profile *profile)
{
	if (likely(profile == &default_root_profile))
		return;
	struct perm_data *p = container_of(profile, struct perm_data,
...
static void migrate_profile(u32 version, struct app_profile *profile)
{
	...
	case 3:
		if (profile->allow_su) {
			profile->rp_config.profile.flags =
