...
int ksu_sulog_handle_compat_dump(void __user *uptr)
{
	...
	if (copy_to_user((void __user *)(uintptr_t)sbuf.uptime_ptr, &uptime,
			 sizeof(uptime)))
		return 1;
	...
	if (copy_to_user((void __user *)(uintptr_t)sbuf.index_ptr, &local_idx,
			 sizeof(local_idx))) {
		kfree(local_buf);
		return 1;
	}

	if (copy_to_user((void __user *)(uintptr_t)sbuf.buf_ptr, local_buf,
			 sizeof(compat_sulog_buf))) {
		kfree(local_buf);
		return 1;
	}
				     ...
				     char *dst, __u32 dst_len)
{
	...
	ret = strncpy_from_user_nofault(dst,
					(const void __user *)untagged_addr(
						(unsigned long)filename_user),
					dst_len);
	if (ret <= 0)
		return ksu_sulog_copy_empty_string(dst);
		  ...
		  const char __user *const __user *argv_user, gfp_t gfp)
{
	...
	filename_len = ksu_sulog_copy_filename(filename_user, filename_buf,
					       min(remaining,
						   KSU_SULOG_MAX_FILENAME_LEN));
	if (!filename_len)
	...
	if (check_add_overflow((__u32)sizeof(*event), filename_len,
			       &payload_len) ||
			    ...
			    gfp_t gfp)
{
	...
	ksu_event_queue_push(&sulog_queue, pending->event_type, 0,
			     pending->payload, pending->payload_len, gfp);
