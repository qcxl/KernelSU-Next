...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0)
typedef void (*ksu_static_call_update_t)(struct static_call_key *key,
...
int ksu_lsm_hook(struct ksu_lsm_hook *hook)
{
	...
	if (scalls_count == 0) {
		...
		if (!kallsyms_lookup_size_offset(scalls_addr, &sym_size,
						 NULL)) {
		...
		if (lsm_active_cnt == 0 || lsm_active_cnt > 20) {
			...
			if (sym_size % (lsm_active_cnt *
			    ...
			    0) {
				pr_debug("invalid struct size\n");
			}
			scalls_count =
				sym_size / sizeof(struct lsm_static_call);
			pr_debug("scalls_count = %zu\n", scalls_count);
		}
	}
	...
	for (i = 0; i < scalls_count; i++) {
		...
		pr_debug("found slot %ld orig %pSb\n", i, current_origin);
		...
		if (!hook->offset) {
			...
			pr_debug("found real slot %ld orig %pSb\n", i,
				 current_origin);
	...
	pr_debug("lsm_hook: patched %s hook slot %px from %px to %px\n",
	...
	if (!kallsyms_lookup_size_offset(heads_addr, &heads_size, NULL)) {
		pr_debug("lookup head size failed");
	}

	head = (struct hlist_head *)heads_addr;
	struct hlist_head *head_end =
		(struct hlist_head *)(heads_addr + heads_size);
	pr_debug(
		"heads_addr 0x%lx head_offset 0x%lx heads_size %ld hook_offset 0x%lx\n",
		(unsigned long)heads_addr, hook->head_offset, heads_size,
		hook->hook_offset);

	for (; head < head_end; head++) {
		hlist_for_each_entry (entry, head, list) {
			void **slot =
				(void **)((char *)entry + hook->hook_offset);
			...
			if (current_origin == target) {
				pr_debug(
					"found %s (target %s) at head offset %ld (provided %ld)\n",
...
void ksu_lsm_unhook(struct ksu_lsm_hook *hook)
{
	...
	if (hook->entry == &hook->list) {
		...
		pr_debug("unhook patch head->first\n");
	} else {
		slot = (void **)((char *)hook->entry + hook->hook_offset);
		pr_debug("unhook patch slot\n");
	}
#endif
	...
	synchronize_rcu();
	pr_debug("lsm_hook: restored %s hook slot %px to %px\n",
...
{
	pr_debug("lsm_hook: init, tracked hooks=%d\n",
