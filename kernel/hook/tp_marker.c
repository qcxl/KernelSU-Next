...
void ksu_mark_all_process(void)
{
	...
	pr_debug("tp_marker: mark all user process done!\n");
}

void ksu_unmark_all_process(void)
{
	handle_process_mark(false);
	pr_debug("tp_marker: unmark all user process done!\n");
}
...
void ksu_mark_running_process_locked(void)
{
	...
	for_each_process_thread (p, t) {
		...
		if (ksu_root_process || is_zygote_process || is_shell ||
			...
			ksu_set_task_tracepoint_flag(t);
			pr_debug(
				"tp_marker: mark process: pid:%d, uid: %d, comm:%s\n",
				t->pid, uid, t->comm);
		} else {
			ksu_clear_task_tracepoint_flag(t);
			pr_debug(
				"tp_marker: unmark process: pid:%d, uid: %d, comm:%s\n",
				t->pid, uid, t->comm);
		}
		put_cred(cred);
	}
...
int ksu_set_task_mark(pid_t pid, bool mark)
{
	...
	if (task) {
		...
		if (mark) {
			...
			pr_debug("tp_marker: marked task pid=%d comm=%s\n", pid,
				 task->comm);
		} else {
			ksu_clear_task_tracepoint_flag(task);
			pr_debug("tp_marker: unmarked task pid=%d comm=%s\n",
				 pid, task->comm);
		}
		put_task_struct(task);
		ret = 0;
