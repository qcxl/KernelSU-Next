			 ...
			 struct perm_datum *perm, int effect, bool invert)
{
	...
	if (src == NULL) {
		...
		if (strip_av(effect, invert)) {
			...
			{
				success &= add_rule_raw(
					db, (struct type_datum *)node->datum,
			...
			ksu_hashtab_for_each(db->p_types.table, node)
			{
				...
				if (type->attribute) {
					success &= add_rule_raw(db, type, tgt,
								cls, perm,
								effect, invert);
				}
			};
		...
		if (strip_av(effect, invert)) {
			ksu_hashtab_for_each(db->p_types.table, node)
			{
				success &= add_rule_raw(
					db, src,
					(struct type_datum *)node->datum, cls,
					perm, effect, invert);
			};
		} else {
			ksu_hashtab_for_each(db->p_types.table, node)
			{
				struct type_datum *type =
					(struct type_datum *)(node->datum);
				if (type->attribute) {
					success &= add_rule_raw(db, src, type,
								cls, perm,
								effect, invert);
				}
			};
