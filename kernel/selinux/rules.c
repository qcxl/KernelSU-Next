...
void apply_kernelsu_rules()
{
	...
	if (IS_ERR(backup_sepolicy)) {
		...
		if (!backup_sepolicy->sidtab) {
			...
			int ret =
				policydb_load_isids(&backup_sepolicy->policydb,
						    backup_sepolicy->sidtab);
			if (ret) {
				...
				pr_debug(
					"backup sepolicy success! latest_granting=%d\n",
	...
	ksu_allow(db, "domain", KERNEL_SU_DOMAIN, "unix_stream_socket",
		  "write");
	ksu_allow(db, "domain", KERNEL_SU_DOMAIN, "unix_stream_socket",
		  "connectto");
	ksu_allow(db, "domain", KERNEL_SU_DOMAIN, "unix_stream_socket",
...
int handle_sepolicy(void __user *user_data, u64 data_len)
{
	...
	if (!getenforce()) {
		pr_debug(
			"SELinux permissive or disabled when handle policy!\n");
	}
